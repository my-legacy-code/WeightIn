...
function toProto(type) {
    ...
    type.declaredMeta().forEach(function (x) {
        ...
        else if (x instanceof metainfo_2.FacetDeclaration) {
 ...
 * @returns {any}
 */
function parse(name, n, r, defaultsToAny, annotation, global) {
    //Build super types.
    if (r === void 0) { r = ts.builtInRegistry(); }
    ...
    if (repeat && repeat.value() == true) {
        ...
        actualResult.addMeta(new restrictions_1.ComponentShouldBeOfType(result));
    }
    if (r instanceof AccumulatingRegistry) {
    ...
    if (repeat) {
        result.meta().forEach(function (x) {
            if (x instanceof metainfo_1.Example) {
                actualResult.addMeta(x);
            }
            if (x instanceof metainfo_1.Examples) {
                actualResult.addMeta(x);
            }
        });
    }
